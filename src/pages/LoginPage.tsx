...
import { Baby, Mail, CreditCard, Eye, EyeOff, ArrowLeft, Shield } from 'lucide-react';
...
export const LoginPage: React.FC<LoginPageProps> = ({ onNavigateToSignup }) => {
  ...
  const [touched, setTouched] = useState({ email: false, idNumber: false });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    login(email.trim(), idNumber.trim());
    setIsLoading(false);
  };

  const isEmailValid = email.includes('@') && email.includes('.');
  const isIdValid = idNumber.length >= 6;
    ...
    <div className="min-h-screen bg-[#E6E6E6] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md animate-fade-in">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-extrabold text-[#404E3B] tracking-tight">أهلاً بكم في تفلي</h1>
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-black/5 border border-[#BAC8B1]/30 p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {/* ID Number Field */}
            <div>
              <label htmlFor="login-id" className="block text-xs font-bold text-[#6C8480] uppercase tracking-wider mb-1.5">
                رقم الهوية الوطنية
              </label>
              <div className="relative">
                <span className="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none text-[#6C8480]">
              ...
              {touched.idNumber && !isIdValid && (
                <p className="text-xs text-red-500 mt-1 font-medium text-right">يجب أن يتكون رقم الهوية من 6 أرقام على الأقل.</p>
              )}
            </div>
            ...
            <div className="bg-[#BAC8B1]/20 border border-[#BAC8B1]/40 rounded-2xl px-4 py-3">
              <p className="text-[11px] text-[#6C8480] font-semibold leading-relaxed text-right">
                <span className="text-[#7B9669] font-bold">الحساب التجريبي:</span> البريد الإلكتروني: <code className="bg-white/70 px-1 rounded">sara@tifli.jo</code> · الهوية: <code className="bg-white/70 px-1 rounded">1234567890</code>
              </p>
            </div>
